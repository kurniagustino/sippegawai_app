...
@section('page-title', 'Upload Berkas Baru')
            ...
            <div class="row">
                <div class="col-md-7">
                    <div class="form-group">
                        <label for="pegawai_id">Pilih Pegawai <span class="text-danger">*</span></label>
                        <select class="form-control @error('pegawai_id') is-invalid @enderror" id="pegawai_id" name="pegawai_id" required>
                            <option value="">-- Pilih Nama Pegawai --</option>
                            @foreach($pegawai as $p)
                            <option value="{{ $p->id }}" {{ old('pegawai_id') == $p->id ? 'selected' : '' }}>{{ $p->nama }} (NIP: {{ $p->nip }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('nama_berkas') is-invalid @enderror" id="nama_berkas" name="nama_berkas" required>
                            ...
                            <option value="KTP" {{ old('nama_berkas') == 'KTP' ? 'selected' : '' }}>KTP (Kartu Tanda Penduduk)</option>
                            <option value="Ijazah SMA/SMK" {{ old('nama_berkas') == 'Ijazah SMA/SMK' ? 'selected' : '' }}>Ijazah SMA/SMK</option>
                            <option value="Ijazah S1" {{ old('nama_berkas') == 'Ijazah S1' ? 'selected' : '' }}>Ijazah S1</option>
                            <option value="Ijazah S2" {{ old('nama_berkas') == 'Ijazah S2' ? 'selected' : '' }}>Ijazah S2</option>
                            <option value="Transkrip Nilai" {{ old('nama_berkas') == 'Transkrip Nilai' ? 'selected' : '' }}>Transkrip Nilai</option>
                            <option value="Sertifikat Keahlian" {{ old('nama_berkas') == 'Sertifikat Keahlian' ? 'selected' : '' }}>Sertifikat Keahlian</option>
                            <option value="Surat Pengalaman Kerja" {{ old('nama_berkas') == 'Surat Pengalaman Kerja' ? 'selected' : '' }}>Surat Pengalaman Kerja</option>
                ...
                <div class="col-md-5">
                    ...
                    {{-- Navigasi PDF (Awalnya tersembunyi) --}}
...
<script>
    ...
    const renderPage = num => {
        ...
        pdfDoc.getPage(num).then(page => {
            ...
            // Buat canvas baru setiap kali render
    ...
    // Fungsi utama untuk menampilkan preview
    function previewFile() {
        const fileInput = document.getElementById('file');
        const fileLabel = document.querySelector('.custom-file-label');
        previewContainer.innerHTML = '';
        pdfControls.classList.add('d-none'); // Sembunyikan navigasi
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = function(e) {
                    ...
                    pdfjsLib.getDocument(typedarray).promise.then(pdf => {
                        ...
                        renderPage(pageNum);
                        pdfControls.classList.remove('d-none'); // Tampilkan navigasi
                    }).catch(err => {
